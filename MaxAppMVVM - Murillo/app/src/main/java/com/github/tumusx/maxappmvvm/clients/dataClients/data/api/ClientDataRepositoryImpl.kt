...
class ClientDataRepositoryImpl @Inject constructor(private val serviceAPI: ClientDataServiceAPI) :
    IClientDataRepository {

    override suspend fun getDataClient(): ApiResult<ClientDTO?> {
        val response = serviceAPI.consultClient()
        if(response.body()?.clientDTO?.contatos?.isEmpty() == true || response.body() == null) return ApiResult.Error("Sem conteúdo a ser exibido. Verificar no ERP")
